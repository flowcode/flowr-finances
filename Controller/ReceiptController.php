...
class ReceiptController extends Controller
{
     ...
     *
     * @Route("/", name="finance_document_rc")
     * @Method("GET")
     * @Template()
     */
    public function indexAction(Request $request)
    {
        ...
        $qb->where('t.name = :type')->setParameter('type', \Flower\FinancesBundle\Entity\DocumentType::TYPE_RECEIPT);
     ...
     *
     * @Route("/{id}/show", name="finance_document_rc_show", requirements={"id"="\d+"})
     * @Method("GET")
     * @Template()
     */
    public function showAction(Document $document)
    {
        $editForm = $this->createForm(new DocumentType(), $document, array(
            'action' => $this->generateUrl('finance_document_rc_update', array('id' => $document->getid())),
            'method' => 'PUT',
        ));
        $deleteForm = $this->createDeleteForm($document->getId(), 'finance_document_rc_delete');
     ...
     *
     * @Route("/new", name="finance_document_rc_new")
     * @Method("GET")
     * @Template()
     */
     ...
     *
     * @Route("/create/sale/{id}", name="finance_document_rc_create_from_sale")
     * @Method("GET")
    ...
    public function createFromSaleAction(Request $request, Sale $sale)
    {
        ...
        return $this->redirect($this->generateUrl('finance_document_rc_show', array('id' => $document->getId())));
     ...
     * Creates a new Document entity.
     *
     * @Route("/create", name="finance_document_rc_create")
     * @Method("POST")
     * @Template("FlowerFinancesBundle:Document:new.html.twig")
     */
     ...
     *
     * @Route("/{id}/edit", name="finance_document_rc_edit", requirements={"id"="\d+"})
     * @Method("GET")
     * @Template()
     */
     ...
     *
     * @Route("/{id}/update", name="finance_document_rc_update", requirements={"id"="\d+"})
     * @Method("PUT")
     ...
     * Save order.
     *
     * @Route("/order/{field}/{type}", name="finance_document_rc_sort")
     */
    public function sortAction($field, $type)
    {
        $this->setOrder('document', $field, $type);

        return $this->redirect($this->generateUrl('finance_document_rc'));
    }
     ...
     *
     * @Route("/{id}/delete", name="finance_document_rc_delete", requirements={"id"="\d+"})
     * @Method("DELETE")
     */
    public function deleteAction(Document $document, Request $request)
    {
        $form = $this->createDeleteForm($document->getId(), 'finance_document_rc_delete');
        if ($form->handleRequest($request)->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->remove($document);
            $em->flush();
